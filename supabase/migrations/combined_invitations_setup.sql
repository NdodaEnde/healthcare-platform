-- Combined SQL migration for invitations system
-- This file contains ALL required database objects for the invitation system to work

-- First, create the system_roles table if it doesn't exist
...
VALUES 
  ('admin', 'Administrator', '["manage:all", "manage:client_organizations"]'::JSONB),
  ('manager', 'Manager', '["view:all", "manage:client_organizations", "manage:reports"]'::JSONB),
  ('doctor', 'Doctor', '["view:patients", "view:tests", "view:certificates", "approve:certificates", "sign:certificates"]'::JSONB),
  ('nurse', 'Nurse', '["view:patients", "create:vitals", "update:vitals", "view:tests"]'::JSONB),
  ('technician', 'Technician', '["view:patients", "create:results", "update:results"]'::JSONB),
  ('receptionist', 'Receptionist', '["view:patients", "create:appointments", "update:appointments"]'::JSONB)
